...
static void G(const uint8_t *k, const uint8_t *a, uint8_t *out_data)
{
	...
	memcpy_s(out_data, MAGMA_DATA_SIZE, a_0, MAGMA_BLOCK_SIZE);
	memcpy_s(out_data + MAGMA_BLOCK_SIZE, MAGMA_DATA_SIZE - MAGMA_BLOCK_SIZE, G, MAGMA_BLOCK_SIZE);
}
...
static void G_Fin(const uint8_t *k, const uint8_t *a, uint8_t *out_data)
{
	...
	memcpy_s(out_data, MAGMA_DATA_SIZE, G, MAGMA_BLOCK_SIZE);
	memcpy_s(out_data + MAGMA_BLOCK_SIZE, MAGMA_DATA_SIZE - MAGMA_BLOCK_SIZE, a_0, MAGMA_BLOCK_SIZE);
}
...
{
	memcpy_s(ctx_dst->key_add1, sizeof(ctx_dst->key_add1), ctx_src->key_add1, MAGMA_ADD_KEY_SIZE);
	memcpy_s(ctx_dst->key_add2, sizeof(ctx_dst->key_add2), ctx_src->key_add2, MAGMA_ADD_KEY_SIZE);
	memcpy_s(ctx_dst->key_orig, sizeof(ctx_dst->key_orig), ctx_src->key_orig, MAGMA_KEY_SIZE);
	memcpy_s(ctx_dst->out, sizeof(ctx_dst->out), ctx_src->out, MAGMA_DATA_SIZE);
}
...
void Magma_Init(magma_ctx_t *ctx, const uint8_t *key)
{
	...
	memset_s(data, sizeof(data), 0x00, MAGMA_DATA_SIZE);
	...
	if (ctx->out[0] & 0x80)
	{
		...
		shl_64(ctx->out, 1);
	memcpy_s(ctx->key_add1, sizeof(ctx->key_add1), ctx->out, MAGMA_ADD_KEY_SIZE);
	...
	if (ctx->out[0] & 0x80)
	{
		...
		shl_64(ctx->out, 1);
	memcpy_s(ctx->key_add2, sizeof(ctx->key_add2), ctx->out, MAGMA_ADD_KEY_SIZE);
}
...
void Magma_MIC(magma_ctx_t *ctx, const uint8_t *blk, uint32_t len)
{
	...
	memset_s(ctx->out, sizeof(ctx->out), 0x00, MAGMA_DATA_SIZE);
	while (len > _len)
	{
		_delta = len - _len;
		...
		else
		{
			uint8_t tmp[MAGMA_DATA_SIZE];
			memset_s(tmp,sizeof(tmp), 0x00, MAGMA_DATA_SIZE);
			memcpy_s(tmp, sizeof(tmp), blk + _len, _delta);
