...
static void E(uint8_t *dst, uint8_t *k, uint8_t *m)
{
	...
	memcpy_s(K, k, STRIBOG_BLOCK_SIZE);
...
static void g_N(uint8_t *h, uint8_t *N, uint8_t *m)
{
	...
	memcpy_s(hash, h, STRIBOG_BLOCK_SIZE);
...
void stribog_calc(stribog_ctx_t *ctx, const uint8_t *message, uint64_t len)
{
	...
	{
		memcpy_s(m, message + len - STRIBOG_BLOCK_SIZE, STRIBOG_BLOCK_SIZE);
	...
	if (padding)
	{
		memset_s(m, 0x00, padding - 1);
		memset_s(m + padding - 1, 0x01, 1);
		memcpy_s(m + padding, message, len);
	}
...
{
	memset_s(ctx->N, 0x00, STRIBOG_BLOCK_SIZE);
	memset_s(ctx->S, 0x00, STRIBOG_BLOCK_SIZE);

	ctx->size = !!size;

	if (ctx->size)
		memset_s(ctx->h, 0x00, STRIBOG_BLOCK_SIZE);
	else
		memset_s(ctx->h, 0x01, STRIBOG_BLOCK_SIZE);
}
